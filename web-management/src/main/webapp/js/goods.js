new Vue({
    ...
    methods: {
        ...
        updateStatus: function (status) {
            let ids = Qs.stringify({ids: this.selectIds}, {indices: false})
            let _this = this;
            axios.get("/goods/updateStatusByIds.do?" + ids + "&status=" + status)
                .then(function (response) {
                    _this.loadAllCategoryList();
                    _this.selectIds = [];
        ...
        loadAllCategoryList: function () {
            let _this = this;
            //发送请求
            axios.post("/itemCate/getItemCategoryList.do")
                .then(function (res) {
                    res = res.data;
                    let list = res.data;
                    if (res.code === 1) {
